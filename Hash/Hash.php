...
class Hash implements IHash
{
    ...
    {
        if (0 > $capacity) {
            throw new HashException('Capacity may not be less than 0');
        }
        ...
        if ('array' === $this->keyType) {
            throw new HashException('Key type may not be type of "array"');
        }
    ...
    public function offsetSet($key, $value): void
    {
        if (count($this->values) >= $this->capacity) {
            throw new HashException("Maximum capacity of '{$this->capacity}' reached.");
        }

        if (null === $key) {
            throw new HashException('Key must be provided.');
        }
        ...
        if (false === self::isValidType($key, $this->keyType, $primitiveKey)) {
            throw new HashException("Invalid key type. Expected {$this->keyType} got {$keyType}");
        }

        if (false === self::isValidType($value, $this->valueType, $primitiveValue)) {
            throw new HashException("Invalid value type. Expected {$this->valueType} got {$valueType}");
        }
